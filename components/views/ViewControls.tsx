...
import { Search, Grid3X3, List, LayoutGrid, SortAsc, Filter, Plus, Upload } from "lucide-react"
...
export default function ViewControls() {
  ...
  const { searchTerm, cardDisplayMode, currentSortOption } = state.currentFocus
    ...
    <div className="space-y-3">
      ...
      <div className="bg-white border border-slate-200 rounded-lg p-4 shadow-sm">
        <div className="flex flex-wrap items-center gap-3 justify-between">
          ...
          {/* Right Section - Enhanced Search Field */}
          <div className="relative flex-shrink-0 w-full sm:w-auto sm:max-w-[300px]">
            <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-slate-400 w-4 h-4" />
            <input
              type="text"
              placeholder="Search actors, gender, ethnicity, skills..."
              value={searchTerm}
              onChange={handleSearchChange}
              className="w-full pl-10 pr-4 py-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-colors text-sm"
            />
            {searchTerm && (
